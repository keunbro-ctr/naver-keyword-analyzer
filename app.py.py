...
<style>
    ...
    .stMetric label { color: #bdc3c7 !important; }
    .stMetric [data-testid="metric-container"] > div { color: #ecf0f1 !important; }
...
# 그래프 키워드 표시/숨김 상태 초기화
if "hidden_keywords" not in st.session_state:
    st.session_state.hidden_keywords = set()
...
# 시각화
# ========================================
def create_interactive_chart(df: pd.DataFrame, hover_mode: str = "closest", hidden_keywords: set = None):
    """인터랙티브 추이 차트 + 가시성 개선(스파이크/툴팁) + 키워드 선택적 표시"""
    if hidden_keywords is None:
        hidden_keywords = set()
    
    # 숨겨진 키워드를 제외한 데이터만 사용
    visible_df = df[~df["keyword"].isin(hidden_keywords)].copy()
    valid_df = visible_df[visible_df["absolute_volume"] > 0].copy()
    if valid_df.empty:
...
def render_keyword_controls(keywords: list, hidden_keywords: set):
    """키워드별 표시/숨김 제어 버튼을 렌더링"""
    st.subheader("🎛️ 키워드 표시 제어")
    
    cols = st.columns(min(len(keywords), 10))
    
    for i, keyword in enumerate(keywords):
        with cols[i % 10]:
            is_hidden = keyword in hidden_keywords
            ...
            with col2:
                if is_hidden:
                    if st.button("➕", key=f"show_{keyword}", help=f"{keyword} 표시"):
                        st.session_state.hidden_keywords.discard(keyword)
                        st.rerun()
                else:
                    if st.button("➖", key=f"hide_{keyword}", help=f"{keyword} 숨김"):
                        st.session_state.hidden_keywords.add(keyword)
                        st.rerun()
...
# ---------------- Sidebar: API Keys (수정된 부분) ----------------
            ...
            if st.button("🔍 분석 시작", type="primary", use_container_width=True):
                ...
                try:
                    with st.spinner("분석 중…"):
                        ...
                        st.session_state.hidden_keywords = set()  # 분석 시작시 숨김 상태 초기화
    ...
    with tab2:
        ...
        if 'final_df' in st.session_state:
            all_keywords = list(st.session_state['final_df']['keyword'].unique())
            ...
            with _col2:
                visible_keywords = [kw for kw in all_keywords if kw not in st.session_state.hidden_keywords]
                focus_kw = st.selectbox("포커스 키워드(선택)", ["(없음)"] + visible_keywords, index=0,
                                        help="선택 시 해당 선만 강조, 나머지는 흐릿하게 표시")

            # 그래프 표시
            fig = create_interactive_chart(st.session_state['final_df'], hover_mode=hover_mode, 
                                           hidden_keywords=st.session_state.hidden_keywords)
            fig = apply_focus(fig, focus_kw)
            if fig:
                st.plotly_chart(fig, use_container_width=True)

            # 키워드 표시/숨김 컨트롤 (그래프 아래로 이동)
            st.markdown("---")
            render_keyword_controls(all_keywords, st.session_state.hidden_keywords)
    ...
    with tab3:
        ...
        if 'recommendations' in st.session_state and not st.session_state['recommendations'].empty:
            ...
            for _, row in rec_df.head(5).iterrows():
                ...
                with c4: st.metric("성장률", f"{row['성장률(%)']:+.1f}%")
                with c5:
                    st.markdown(f"""
                    <div class='grade-card {grade_class}' style='height: 70px; display: flex; flex-direction: column; justify-content: center;'>
                        <div style='font-size: 14px; margin-bottom: 3px;'>{row['등급']}</div>
                        <div style='font-size: 20px;'>{int(row['총점'])}점</div>
                    </div>
                    """, unsafe_allow_html=True)
                st.markdown("---")
